...
void display(char grid[max_row][max_col], char *grid_name)
{
	...
	for (i = 0; i < max_row; i++) {
		...
		for (j = 0; grid[i][j]; j++) {
			switch (grid[i][j]) {
			case WALL:
				display_in_color(height, tmp_width++, COLOR_RED, '#');
				break;
			case GOAL:
				display_in_color(height, tmp_width++, COLOR_YELLOW, '.');
				break;
			case BOX:
				display_in_color(height, tmp_width++, COLOR_MAGENTA, '$');
				break;
			case BOX_ON_GOAL:
				display_in_color(height, tmp_width++, COLOR_GREEN, '*');
				break;
			case MAN:
				display_in_color(height, tmp_width++, COLOR_WHITE, '@');
				break;
			case FLOOR:
				display_in_color(height, tmp_width++, COLOR_WHITE, ' ');
				break;
			case MAN_ON_GOAL:
				display_in_color(height, tmp_width++, COLOR_CYAN, '+');
				break;				
			default:
				mvprintw(height, tmp_width++, "%c", grid[i][j]);
			}
		}
	}
	endwin();
